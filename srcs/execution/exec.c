...
/*   Updated: 2025/01/06 19:11:32 by cgorin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		char **env_)
{
	...
	if (execve(cmd->args[0], cmd->args, env_) == -1)
	{
		if (ft_strchr(cmd->args[0], '/') && stat(cmd->args[0], &statbuf) == 0
			...
			error(p, cmd->args[0], "Is a directory", 126);
		if (ft_strchr(cmd->args[0], '/') == NULL)
			error(p, cmd->args[0], "command not found", 127);
		else if (access(cmd->args[0], F_OK) == 0 && access(cmd->args[0],
				X_OK) != 0)
			error(p, cmd->args[0], "Permission denied", 126);
		else
			error(p, cmd->args[0], "No such file or directory", 127);
	}
	if (!cmd->path)
		error(p, cmd->args[0], "command not found", 127);
	exit(1);
